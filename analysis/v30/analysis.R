...
dfs %>%   filter(Stotal>1000) %>% filter(Plate_ID=='Plate1') %>% filter(virus_identity=='Contrived' & lysate=='Saliva') %>%
...
ggsave(paste0(outdir,'plat1_v26_plate1.png'))
 

dfs %>%   filter(Stotal>1000) %>% filter(Plate_ID!='Plate1')  %>% filter(lysate=='ED Saliva' | lysate=='Nasal') %>%
    ...
    theme_bw()+
     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
ggsave(paste0(outdir,'ED_saliva_and_AsheNasalSwabs.png'))
