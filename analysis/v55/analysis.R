...
#taqpath saliva gamma LoD v54

dfs %>% filter(quadrant_96=='A') %>% 
    filter(Plate_ID=='Plate4') %>% filter(virus_identity!='0') %>%
    # filter(virus_identity!='TE') %>%
    #filter(Stotal>500) %>%
...
geom_quasirandom(size=2, alpha=.75)+
    ...
    #facet_grid(~NPResult, scales='free_x')+
    theme(axis.text.x = element_text(angle = 90))+
    ggtitle('Gamma Saliva LoD _ Taqpath index plate 4 v54')


dfs %>% filter(quadrant_96=='A') %>% 
    filter(Plate_ID=='Plate4') %>% filter(virus_identity!='0') %>% write.csv(paste0(rundir, 'report_saliva_LOD.csv'))
...
dfsFC=dfsF %>% filter(NPResult!='NA') %>% filter(quadrant_96 !='A') %>% filter(quadrant_96 !='B') %>%  filter(quadrant_96 =='C')
dfsFD=dfsF %>% filter(NPResult!='NA') %>% filter(quadrant_96 !='A') %>% filter(quadrant_96 !='B') %>%  filter(quadrant_96 =='D')
dfsF %>% filter(NPResult!='NA') %>% write.csv(paste0(rundir, 'report_MNS_NS_patient_samples_v54.csv'))

par(mfrow=c(1,3))
plot(log10(dfsFC$S2_normalized_to_S2_spike), log10(dfsFD$S2_normalized_to_S2_spike), xlab='C', ylab='D', main='log10(S2_normalized)', col=as.factor(dfsFC$NPResult))

plot(log10(dfsFC$S2+1), log10(dfsFD$S2+1),xlab='C', ylab='D', main='log10(S2+1)', col=as.factor(dfsFC$NPResult))
plot(log10(dfsFC$S2_spike+1), log10(dfsFD$S2_spike+1),xlab='C', ylab='D', main='log10(S2_spike+1)', col=as.factor(dfsFC$NPResult))


dfsFA=dfsF %>% filter(NPResult!='NA') %>% filter(quadrant_96 !='A') %>% filter(quadrant_96 !='B') %>%  filter(quadrant_96 =='D')
gt=ggplot(dfsFA, aes(x=Ct, y=S2_normalized_to_S2_spike, color=log10(Stotal), label=virus_identity))+
geom_quasirandom(size=2) + scale_color_viridis_c(option = 'plasma')+
...
gt2$widths[7]=.3*gt2$widths[7]
grid.draw(gt2)
ggsave(paste0(outdir,'MNS_NS_taqpath.png'))



dfsFA %>% write.csv(paste0(outdir,'MNS_NS_taqpath.csv'))
