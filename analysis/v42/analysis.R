...
titl='v42 - Ashe TE + saliva neg + tube test'
...
ggsave(paste0(outdir,'NS_MNS_1_4_LoD.png'))

dfs %>%  filter(Plate_ID=='Plate5')  %>% filter(lysate!='TE') %>% filter(Col=='02' | Col=='06' | Col=='08' | Col=='12') %>%
    ggplot(aes(x=virus_copy, y=S2_normalized_to_S2_spike, color=Stotal>1000))+
    #geom_point(alpha=.75, size=2)+
    geom_quasirandom(alpha=.75, size=2)+
    facet_wrap(~lysate, scales='free_x')+
 ...
 dfs %>%  filter(Plate_ID=='Plate5')  %>% filter(lysate!='TE') %>%
    ...
    theme_bw()+
     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ggtitle('NS MNS 1:4 LoD')
ggsave(paste0(outdir,'NS_MNS_1_4_LoD_S2.png'))

 library(gdata)
pd=read.xls(paste0(outdir, 'EUA_data.xlsx'), stringsAsFactors=F)
pd$Sample[2]='R-003'
pd$Sample[19]='R-006'
